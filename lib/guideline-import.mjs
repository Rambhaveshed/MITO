export const GUIDELINE_IMPORT_SCHEMA_VERSION = "1.1.0";

const allowedUnits = new Set(["sqft", "sqm", "cent", "ground", "acre"]);
const allowedCaptureMethods = new Set(["official_export", "manual_official_snapshot", "official_api", "archived_official_snapshot"]);
...
export function validateGuidelineImport(batch, options = {}) {
  ...
  if (!isObject(batch.source)) {
    ...
    if (batch.source.captureMethod === "archived_official_snapshot") {
      pushRequiredString(errors, batch.source.officialSourceUrl, "source.officialSourceUrl");
      pushRequiredString(errors, batch.source.archivedBy, "source.archivedBy");
      if (!isNonEmptyString(batch.source.snapshotAsOf) || Number.isNaN(Date.parse(batch.source.snapshotAsOf))) {
        errors.push("source.snapshotAsOf must be an ISO date-time for archived official snapshots");
      }
      if (batch.source.evidenceForm !== "portal_screenshot_in_project_file") {
        errors.push("source.evidenceForm must identify the archived project-file screenshot");
      }
    }
  }
  ...
  batch.rows.forEach((row, index) => {
    ...
    for (const field of ["sourceRecordId", "officialSroCode", "officialVillageCode", "sourceStreetName", "classification", "locationEvidence"]) {
      pushRequiredString(errors, row[field], `${path}.${field}`);
    }
    if (!Object.hasOwn(row, "officialStreetCode")) errors.push(`${path}.officialStreetCode must be present`);
    if (row.officialStreetCode !== null && !isNonEmptyString(row.officialStreetCode)) {
      errors.push(`${path}.officialStreetCode must be a non-empty string or null`);
    }
    ...
    if (row.verificationStatus === "verified" && !isNonEmptyString(row.officialStreetCode)) {
      errors.push(`${path}.officialStreetCode is required before a record can be verified`);
    }
    if (row.officialStreetCode === null && !row.qualityFlags?.includes("official_street_code_not_visible")) {
      errors.push(`${path}.qualityFlags must explain why the official street code is unavailable`);
    }
