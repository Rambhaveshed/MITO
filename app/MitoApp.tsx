...
import {
  ...
  omrSemmancheriAuctionPoint,
  omrSemmancheriAuctionPointCenter,
...
const SEMMANCHERI_VILLAGE_KEY = "20066:246";
...
const SEMMANCHERI_AUCTION_SOURCE_ID = "tmb-semmancheri-industrial-land-point";
const SEMMANCHERI_AUCTION_LAYER_ID = "tmb-semmancheri-industrial-land-point-circle";
...
export default function MitoApp() {
  ...
  const selectedCoverageSecuredCreditorLandReserves = selectedCoverageUnit
    ...
    : [];
  const selectedCoverageNavalurResidentialLandReserves = selectedCoverageUnit
    ? navalurResidentialLandReservesForVillage(selectedCoverageUnit.officialSroCode, selectedCoverageUnit.officialVillageCode)
    : [];
  ...
  const selectedCoverageUnresolvedThaiyurLandReserves = selectedCoverageUnit
    ...
    : [];
  const selectedCoverageAcceptedLandBidPrices = selectedCoverageUnit
    ? semmancheriAcceptedLandBidPricesForVillage(selectedCoverageUnit.officialSroCode, selectedCoverageUnit.officialVillageCode)
    : [];
  const selectedCoverageLatestSecuredCreditorLandReserve = selectedCoverageSecuredCreditorLandReserves.length
    ...
    : null;
  const selectedCoverageLatestNavalurResidentialLandReserve = selectedCoverageNavalurResidentialLandReserves.length
    ? selectedCoverageNavalurResidentialLandReserves[selectedCoverageNavalurResidentialLandReserves.length - 1]
    : null;
  ...
  const selectCoverageUnit = (officialSroCode: string, officialVillageCode: string) => {
    ...
    } else if (key === SEMMANCHERI_VILLAGE_KEY) {
      mapRef.current?.flyTo({ center: omrSemmancheriAuctionPointCenter, zoom: 15.2, duration: 900 });
    }
  };
  ...
  useEffect(() => {
    ...
    void import("maplibre-gl").then((maplibre) => {
      ...
      map.on("load", () => {
        ...
        map.addSource(SEMMANCHERI_AUCTION_SOURCE_ID, {
          type: "geojson",
          data: omrSemmancheriAuctionPoint,
        });
        map.addLayer({
          id: SEMMANCHERI_AUCTION_LAYER_ID,
          type: "circle",
          source: SEMMANCHERI_AUCTION_SOURCE_ID,
          paint: {
            "circle-radius": 7,
            "circle-color": "#9a5a18",
        ...
        map.on("click", SEMMANCHERI_AUCTION_LAYER_ID, () => selectCoverageUnit("20066", "246"));
        map.on("mouseenter", SEMMANCHERI_AUCTION_LAYER_ID, () => { map.getCanvas().style.cursor = "pointer"; });
        map.on("mouseleave", SEMMANCHERI_AUCTION_LAYER_ID, () => { map.getCanvas().style.cursor = ""; });
  ...
  useEffect(() => {
    ...
    const isSiruseriSelected = selectedCoverageKey === SIRUSERI_VILLAGE_KEY;
    const isSemmancheriSelected = selectedCoverageKey === SEMMANCHERI_VILLAGE_KEY;
    for (const layerId of [SIRUSERI_FILL_LAYER_ID, SIRUSERI_LINE_LAYER_ID]) {
      map.setLayoutProperty(layerId, "visibility", isVisible ? "visible" : "none");
    }
    map.setLayoutProperty(SEMMANCHERI_AUCTION_LAYER_ID, "visibility", isVisible ? "visible" : "none");
    map.setPaintProperty(SIRUSERI_FILL_LAYER_ID, "fill-opacity", isSiruseriSelected ? 0.25 : 0.16);
    map.setPaintProperty(SIRUSERI_LINE_LAYER_ID, "line-width", isSiruseriSelected ? 3 : 2);
    map.setPaintProperty(SEMMANCHERI_AUCTION_LAYER_ID, "circle-radius", isSemmancheriSelected ? 9 : 7);
    ...
    <main className="mito-shell">
      ...
      <div className="map-legend">
        {activeView === "Coverage" ? (
          <>
            {coverageScope === "omr"
              ? <span><i className="legend-approx" /> 1 approximate footprint · 1 source-published asset point</span>
                ...
                ? selectedCoverageUnit
                  ? selectedCoverageAuctionReservePrices.length
                    ...
                    : selectedCoverageLatestSecuredCreditorLandReserve
                      ? <><strong>{formatRate(selectedCoverageLatestSecuredCreditorLandReserve.derivedReservePriceInrPerSqft)}</strong><span>per sq ft · latest land-auction reserve</span></>
          ...
          {activeView === "Coverage" && coverageScope === "omr" && inspectorTab === "Overview" && (
            selectedCoverageUnit && selectedCoverageInventory ? (
              <>
                ...
                {selectedCoverageSecuredCreditorLandReserves.length > 0 && (
                  <section className="auction-evidence-card">
                    <div><MapPin size={17} /></div>
                    <p><strong>{selectedCoverageSecuredCreditorLandReserves.length} official industrial-land reserve events</strong><span>₹1.96 crore → ₹1.7955 crore · 8,017 sq ft · source-published asset point</span></p>
                    <em>Reserve price, not sale</em>
                  </section>
                )}

                {selectedCoverageNavalurResidentialLandReserves.length > 0 && (
                  <section className="auction-evidence-card">
                    <div><Database size={17} /></div>
                    <p><strong>{selectedCoverageNavalurResidentialLandReserves.length} document-backed Navalur land reserves</strong><span>₹57.365 lakh → ₹45.00 lakh · 1,043 sq ft · unplotted</span></p>
                    <em>Historical floors, not a complete history or sale</em>
                  </section>
                )}

                {selectedCoverageKazhipatturCombinedAssetReserves.length > 0 && (
                  <section className="auction-evidence-card">
                    <div><Building2 size={17} /></div>
                    <p><strong>{selectedCoverageKazhipatturCombinedAssetReserves.length} official Kazhipattur combined-asset reserves</strong><span>2 land-and-building assets · 3 verified checkpoints each · unplotted</span></p>
                ...
                {selectedCoverageAcceptedLandBidPrices.length > 0 && (
                  <section className="auction-evidence-card">
                    ...
                    <p><strong>1 accepted Semmancheri land-auction bid</strong><span>₹68.70 lakh · 2,108 sq ft · later cancelled and deposit forfeited</span></p>
                    <em>Accepted bid, not completed sale</em>
                  </section>
                )}
                ...
                {selectedCoverageSecuredCreditorLandReserves.length > 0 && (
                  <section className="inspector-section auction-reserve-section">
                    <div className="section-heading"><div><span>Official secured-creditor auctions</span><h2>Semmancheri industrial-land reserve timeline</h2></div><MapPin size={17} /></div>
                    {selectedCoverageSecuredCreditorLandReserves.map((record) => (
                      <article key={record.id} className="auction-reserve-card">
                        <div><span>industrial land</span><em>{record.auctionDate === "2024-09-11" ? "auction" : "re-auction"}</em></div>
                        <strong>{formatRate(record.derivedReservePriceInrPerSqft)}<small>/land sq ft</small></strong>
                        <p>₹{(record.totalReservePriceInr / 10_000_000).toFixed(4).replace(/0+$/, "").replace(/\.$/, "")} crore reserve · {inr.format(record.landAreaSqft)} sq ft land</p>
                        <footer><span>Auction {new Date(`${record.auctionDate}T00:00:00+05:30`).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span><b>Source point</b></footer>
                      </article>
                    ))}
                    <p className="coverage-method">The same survey group, area and six-decimal coordinate recur across all five notices. The reserve fell 8.39%, from ₹2,444.80 to ₹2,239.62 per land sq ft. TMB&apos;s 30 June 2026 secured-assets register still lists the land as security possessed; that is lifecycle evidence, not proof of current availability or a failed auction.</p>
                  </section>
                )}

                {selectedCoverageNavalurResidentialLandReserves.length > 0 && (
                  <section className="inspector-section auction-reserve-section">
                    <div className="section-heading"><div><span>Document-backed statutory notices</span><h2>Navalur residential-land reserve checkpoints</h2></div><Database size={17} /></div>
                    {selectedCoverageNavalurResidentialLandReserves.map((record) => (
                      <article key={record.id} className="auction-reserve-card">
                        <div><span>residential land</span><em>{record.auctionDate === "2025-07-22" ? "auction" : "re-auction"}</em></div>
                        <strong>{formatRate(record.derivedReservePriceInrPerSqft)}<small>/land sq ft</small></strong>
                        <p>₹{(record.totalReservePriceInr / 100_000).toFixed(2)} lakh reserve · {inr.format(record.landAreaSqft)} sq ft land</p>
                    ...
                    <p className="coverage-method">Two inspected Business Standard copies of Tata-issued statutory notices support these checkpoints. The verified floor moved from ₹5,500 to ₹4,314.48 per land sq ft, 21.56% lower. Five aggregator-only claims are excluded pending their underlying notices, so this is not presented as a complete reserve history.</p>
                  </section>
                )}

                {selectedCoverageKazhipatturCombinedAssetReserves.length > 0 && (
                  <section className="inspector-section auction-reserve-section">
                    <div className="section-heading"><div><span>Official SBI secured-creditor notices</span><h2>Kazhipattur combined-asset reserve checkpoints</h2></div><Building2 size={17} /></div>
                    {selectedCoverageKazhipatturCombinedAssetReserves.map((record) => {
                      const asset = omrKazhipatturCombinedAssetReserveLedger.assets.find((candidate) => candidate.id === record.assetId)!;
                      return (
                        <article key={record.id} className="auction-reserve-card">
                          <div><span>{asset.streetName}</span><em>combined land + building</em></div>
                          <strong>{formatRate(record.derivedCombinedReserveDividedByLandAreaInrPerSqft)}<small>/land-area sq ft</small></strong>
                          <p>₹{(record.totalReservePriceInr / 100_000).toFixed(0)} lakh combined reserve · {inr.format(record.landAreaSqft)} sq ft land</p>
                          <footer><span>Auction {new Date(`${record.auctionDate}T00:00:00+05:30`).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span><b>Unplotted</b></footer>
                        </article>
                      );
                    })}
                    <p className="coverage-method">These comparison-only densities divide each land-and-building reserve by the published land area. Asset 1&apos;s verified floor is 4.96% lower at the latest checkpoint; Asset 2&apos;s is 15.87% higher. Neither movement is a land-price or market trend, and the verified notices are not a complete reserve history.</p>
                  </section>
                )}
                ...
                <section className="inspector-section">
                  ...
                  {selectedCoverageSecuredCreditorLandReserves.length > 0 && <div className="gate-row passed"><Check size={14} /><span>Five official Semmancheri industrial-land reserve observations and one source point reconciled</span></div>}
                  {selectedCoverageSecuredCreditorLandReserves.length > 0 && <div className="gate-row"><X size={14} /><span>Winning bid, sale certificate, registered transfer, current availability and parcel boundary are not published</span></div>}
                  {selectedCoverageNavalurResidentialLandReserves.length > 0 && <div className="gate-row passed"><Check size={14} /><span>Two Navalur statutory-notice copies reconciled to one 1,043 sq-ft residential plot</span></div>}
            ...
            <>
              ...
              <section className="auction-evidence-card">
                <div><MapPin size={17} /></div>
                <p><strong>{omrOfficialSecuredCreditorLandReserveSummary.recordCount} Semmancheri industrial-land reserve events</strong><span>One 8,017 sq-ft asset · ₹1.96 crore → ₹1.7955 crore · source point, no sale outcome</span></p>
                <em>Reserve price, not sale</em>
              </section>

              <section className="auction-evidence-card">
                <div><Database size={17} /></div>
                <p><strong>{omrNavalurResidentialLandReserveSummary.recordCount} Navalur residential-land reserve checkpoints</strong><span>One 1,043 sq-ft plot · ₹57.365 lakh → ₹45 lakh · unplotted, outcome unknown</span></p>
                <em>Document-backed floors · incomplete history</em>
              </section>

              <section className="auction-evidence-card">
                <div><Building2 size={17} /></div>
                <p><strong>{omrKazhipatturCombinedAssetReserveSummary.recordCount} Kazhipattur combined-asset reserve checkpoints</strong><span>Two land-and-building assets · three official observations each · zero land-rate records</span></p>
              ...
              <section className="auction-evidence-card">
                <div><Check size={17} /></div>
                <p><strong>{omrSemmancheriAcceptedLandBidOutcomeSummary.acceptedBidRecordCount} accepted Semmancheri land bid</strong><span>₹68.70 lakh · ₹3,259.01/land sq ft · cancelled before completion</span></p>
                <em>Accepted bid · not registered sale</em>
              </section>
          ...
          {activeView === "Coverage" && coverageScope === "omr" && inspectorTab === "Evidence" && (
            selectedCoverageUnit && selectedCoverageInventory ? (
              <>
                ...
                {selectedCoverageSecuredCreditorLandReserves.length > 0 && (
                  <section className="inspector-section evidence-section auction-source-section">
                    <div className="section-heading"><div><span>Official secured-creditor evidence</span><h2>TMB Semmancheri source chain</h2></div><MapPin size={17} /></div>
                    {omrOfficialSecuredCreditorLandReserveLedger.sources.map((source) => (
                      <a key={source.id} href={source.url} target="_blank" rel="noreferrer" className="source-card auction-source">
                        <span className="source-kind official">official</span>
                        <div><strong>{source.title}</strong><p>{source.organization}</p><small>{("auctionDate" in source ? `auction ${source.auctionDate}` : `effective ${source.effectiveDate}`)} · pages {source.pagesInspected.join(", ")} · factual excerpt only</small></div>
                        <ExternalLink size={15} />
                      </a>
                    ))}
                    {selectedCoverageSecuredCreditorLandReserves.map((record) => (
                      <article key={record.id} className="auction-reserve-card detailed">
                        <div><span>{record.priceTypeLabel}</span><em>{record.lifecycleStatus.replaceAll("_", " ")}</em></div>
                        <strong>{formatRate(record.derivedReservePriceInrPerSqft)}<small>/land sq ft</small></strong>
                        <p>₹{(record.totalReservePriceInr / 10_000_000).toFixed(4).replace(/0+$/, "").replace(/\.$/, "")} crore reserve divided by {inr.format(record.landAreaSqft)} sq ft.</p>
                        <footer><span>Winning bid: unpublished</span><b>Direct village · source point</b></footer>
                      </article>
                    ))}
                ...
                {selectedCoverageNavalurResidentialLandReserves.length > 0 && (
                  <section className="inspector-section evidence-section auction-source-section">
                    <div className="section-heading"><div><span>Document-backed secured-creditor evidence</span><h2>Tata Navalur source chain</h2></div><Database size={17} /></div>
                    {omrNavalurResidentialLandReserveLedger.sources.map((source) => (
                      <a key={source.id} href={source.url} target="_blank" rel="noreferrer" className="source-card auction-source">
                        <span className={`source-kind ${source.officialHost ? "official" : "snapshot"}`}>{source.officialHost ? "official feed" : "notice copy"}</span>
                        <div><strong>{source.title}</strong><p>{source.organization}</p><small>{"auctionDate" in source ? `auction ${source.auctionDate} · page ${source.pagesInspected.join(", ")}` : `${source.recordCountScanned} current-feed rows audited · ${source.targetMatchCount} target matches`} · factual audit only</small></div>
                        <ExternalLink size={15} />
                      </a>
                    ))}
                    {selectedCoverageNavalurResidentialLandReserves.map((record) => (
                      <article key={record.id} className="auction-reserve-card detailed">
                        <div><span>{record.priceTypeLabel}</span><em>{record.lifecycleStatus.replaceAll("_", " ")}</em></div>
                        <strong>{formatRate(record.derivedReservePriceInrPerSqft)}<small>/land sq ft</small></strong>
                        <p>₹{(record.totalReservePriceInr / 100_000).toFixed(2)} lakh reserve divided by {inr.format(record.landAreaSqft)} sq ft.</p>
                        <footer><span>Winning bid: unpublished</span><b>Direct village · unplotted</b></footer>
                      </article>
                    ))}
                    <p className="coverage-method">The copies reproduce Tata-issued statutory notices, but their host is not promoted as an official Tata source. The official 12 August 2026 Tata export had zero target matches; absence cannot establish that the plot sold, was withdrawn or is currently available. Five secondary claims remain unnormalized.</p>
                  </section>
                )}

                {selectedCoverageKazhipatturCombinedAssetReserves.length > 0 && (
                  <section className="inspector-section evidence-section auction-source-section">
                    <div className="section-heading"><div><span>Official secured-creditor evidence</span><h2>SBI Kazhipattur source chain</h2></div><Building2 size={17} /></div>
                    {omrKazhipatturCombinedAssetReserveLedger.sources.map((source) => (
                      <a key={source.id} href={source.url} target="_blank" rel="noreferrer" className="source-card auction-source">
                        <span className="source-kind official">official</span>
                        <div><strong>{source.title}</strong><p>{source.organization}</p><small>notice {source.noticeDate} · auction {source.auctionDate} · pages {source.pagesInspected.join(", ")} · factual excerpt only</small></div>
                        <ExternalLink size={15} />
                      </a>
                    ))}
                    {selectedCoverageKazhipatturCombinedAssetReserves.map((record) => {
                      ...
                      return (
                        <article key={record.id} className="auction-reserve-card detailed">
                          <div><span>{asset.streetName} combined asset</span><em>{record.lifecycleStatus.replaceAll("_", " ")}</em></div>
                          <strong>{formatRate(record.derivedCombinedReserveDividedByLandAreaInrPerSqft)}<small>/land-area sq ft</small></strong>
                          <p>₹{(record.totalReservePriceInr / 100_000).toFixed(0)} lakh combined reserve divided by {inr.format(record.landAreaSqft)} sq ft land.</p>
                          <footer><span>Winning bid: unpublished</span><b>Direct village · unplotted</b></footer>
                        </article>
                      );
                    })}
                ...
                {selectedCoverageAcceptedLandBidPrices.length > 0 && (
                  <section className="inspector-section evidence-section auction-source-section">
                    <div className="section-heading"><div><span>Tribunal-stated auction outcome</span><h2>Semmancheri accepted-bid chain</h2></div><Check size={17} /></div>
                    {omrSemmancheriAcceptedLandBidOutcomeLedger.sources.map((source) => (
                      <a key={source.id} href={source.url} target="_blank" rel="noreferrer" className="source-card auction-source">
                        <span className={`source-kind ${source.sourceType.startsWith("official") ? "official" : "snapshot"}`}>{source.sourceType.startsWith("official") ? "official docket" : "order mirror"}</span>
                        <div><strong>{source.title}</strong><p>{source.organization}</p><small>{"orderDate" in source ? `order ${source.orderDate}` : `cause list ${source.causeListDate}`} · factual excerpt only</small></div>
                        <ExternalLink size={15} />
                      </a>
                    ))}
                    {selectedCoverageAcceptedLandBidPrices.map((record) => (
                      <article key={record.id} className="auction-reserve-card detailed">
                        <div><span>{record.isAcceptedBid ? "accepted auction bid" : "auction reserve"}</span><em>{record.isAcceptedBid ? "cancelled after unpaid balance" : "auction floor"}</em></div>
                        <strong>{formatRate(record.derivedPriceInrPerSqft)}<small>/land sq ft</small></strong>
                        <p>₹{(record.totalPriceInr / 100_000).toFixed(2)} lakh divided by {inr.format(record.landAreaSqft)} sq ft.</p>
                        <footer><span>{record.isAcceptedBid ? "Accepted 27 Mar 2024" : "Reserve ₹68 lakh"}</span><b>{record.isAcceptedBid ? "No completed sale" : "Direct village · unplotted"}</b></footer>
                      </article>
                    ))}
                    <p className="coverage-method">The mirrored order says the ₹68.70 lakh bid was accepted and confirmed, but only 25% was paid; the bank cancelled the sale and forfeited the deposit on 31 August 2024. The official DRT docket confirms the case identity but labels IA 447 differently from the order mirror, so that conflict remains explicit.</p>
                  </section>
                )}
            ...
            <>
              <section className="inspector-section evidence-section">
                ...
                <a href={omrOfficialSecuredCreditorLandReserveLedger.sources[4].url} target="_blank" rel="noreferrer" className="source-card auction-source">
                  <span className="source-kind official">official</span>
                  <div><strong>{omrOfficialSecuredCreditorLandReserveLedger.sources[4].title}</strong><p>{omrOfficialSecuredCreditorLandReserveLedger.sources[4].organization}</p><small>5 Semmancheri industrial-land reserves · 1 source point · latest reserve ₹2,239.62/land sq ft · no sale outcome</small></div>
                  <ExternalLink size={15} />
                </a>
                <a href={omrOfficialSecuredCreditorLandReserveLedger.sources[5].url} target="_blank" rel="noreferrer" className="source-card auction-source">
                  <span className="source-kind official">lifecycle</span>
                ...
                <a href={omrNavalurResidentialLandReserveLedger.sources[1].url} target="_blank" rel="noreferrer" className="source-card auction-source">
                  <span className="source-kind snapshot">notice copy</span>
                  <div><strong>{omrNavalurResidentialLandReserveLedger.sources[1].title}</strong><p>{omrNavalurResidentialLandReserveLedger.sources[1].organization}</p><small>2 document-backed Navalur reserve checkpoints · ₹5,500 → ₹4,314.48/land sq ft · incomplete history, no sale outcome</small></div>
                  <ExternalLink size={15} />
                </a>
                <a href={omrNavalurResidentialLandReserveLedger.sources[2].url} target="_blank" rel="noreferrer" className="source-card auction-source">
                  <span className="source-kind official">official feed</span>
                  <div><strong>{omrNavalurResidentialLandReserveLedger.sources[2].title}</strong><p>{omrNavalurResidentialLandReserveLedger.sources[2].organization}</p><small>511 rows audited on 12 Aug 2026 · zero target matches · absence does not prove sale, withdrawal or current status</small></div>
                  <ExternalLink size={15} />
                </a>
                <a href={omrKazhipatturCombinedAssetReserveLedger.sources[2].url} target="_blank" rel="noreferrer" className="source-card auction-source">
                  <span className="source-kind official">official</span>
                ...
                <a href={omrSemmancheriAcceptedLandBidOutcomeLedger.sources[1].url} target="_blank" rel="noreferrer" className="source-card auction-source">
                  <span className="source-kind snapshot">order mirror</span>
                  <div><strong>{omrSemmancheriAcceptedLandBidOutcomeLedger.sources[1].title}</strong><p>{omrSemmancheriAcceptedLandBidOutcomeLedger.sources[1].organization}</p><small>₹68 lakh reserve · ₹68.70 lakh accepted bid · cancelled and deposit forfeited · no completed sale</small></div>
                  <ExternalLink size={15} />
                </a>
          ...
          {activeView === "Coverage" && coverageScope === "omr" && inspectorTab === "Context" && (
            selectedCoverageUnit && selectedCoverageInventory ? (
              <>
                ...
                {selectedCoverageSecuredCreditorLandReserves.length > 0 && (
                  <section className="inspector-section context-section">
                    <div className="section-heading"><div><span>Price interpretation</span><h2>What the Semmancheri reserve history means</h2></div><CircleHelp size={17} /></div>
                    <dl className="detail-grid">
                      <div><dt>Evidence</dt><dd>Industrial-land auction floors</dd></div>
                      <div><dt>Observations</dt><dd>5 · Sep 2024 to Dec 2025</dd></div>
                      <div><dt>Asset</dt><dd>Three parcels · 8,017 sq ft</dd></div>
                      <div><dt>Reserve change</dt><dd>−{omrOfficialSecuredCreditorLandReserveSummary.reserveReductionPercent}%</dd></div>
                      <div><dt>Lifecycle</dt><dd>Security possessed at 30 Jun 2026</dd></div>
                      <div><dt>Geometry</dt><dd>Source point · parcel boundary absent</dd></div>
                    </dl>
                ...
                {selectedCoverageNavalurResidentialLandReserves.length > 0 && (
                  <section className="inspector-section context-section">
                    <div className="section-heading"><div><span>Price interpretation</span><h2>What the Navalur checkpoints mean</h2></div><CircleHelp size={17} /></div>
                    <dl className="detail-grid">
                      <div><dt>Evidence</dt><dd>Residential-land auction floors</dd></div>
                      <div><dt>Verified points</dt><dd>2 · Jul and Oct 2025</dd></div>
                      <div><dt>Asset</dt><dd>Park Enclave plot · 1,043 sq ft</dd></div>
                      <div><dt>Checkpoint change</dt><dd>−{omrNavalurResidentialLandReserveSummary.verifiedCheckpointReductionPercent}%</dd></div>
                      <div><dt>Outcome/current status</dt><dd>Not published</dd></div>
                      <div><dt>Geometry</dt><dd>Unplotted</dd></div>
                    </dl>
                    <p className="coverage-method">The verified checkpoints moved from ₹57.365 lakh to ₹45 lakh. That shows a lower later auction floor for the same plot, not a market-price decline or sale result. Five intervening or later aggregator claims are visible as capture gaps and excluded from calculations.</p>
                    <a className="resolver-api-link" href={omrNavalurResidentialLandReserveLedger.sources[0].url} target="_blank" rel="noreferrer">Open the July statutory-notice copy <ArrowUpRight size={13} /></a>
                    <a className="resolver-api-link" href={omrNavalurResidentialLandReserveLedger.sources[2].url} target="_blank" rel="noreferrer">Open the current official Tata feed <ArrowUpRight size={13} /></a>
                  </section>
                )}

                {selectedCoverageKazhipatturCombinedAssetReserves.length > 0 && (
                  <section className="inspector-section context-section">
                    <div className="section-heading"><div><span>Price interpretation</span><h2>What the Kazhipattur reserve changes mean</h2></div><CircleHelp size={17} /></div>
                    <dl className="detail-grid">
                      <div><dt>Evidence</dt><dd>Combined land-and-building auction floors</dd></div>
                      <div><dt>Assets</dt><dd>{omrKazhipatturCombinedAssetReserveSummary.assetCount}</dd></div>
                      <div><dt>Checkpoints</dt><dd>3 per asset · May 2023 to Jan 2025</dd></div>
                      <div><dt>Asset 1 change</dt><dd>{omrKazhipatturCombinedAssetReserveSummary.assetComparisons[0].verifiedCheckpointChangePercent}%</dd></div>
                      <div><dt>Asset 2 change</dt><dd>+{omrKazhipatturCombinedAssetReserveSummary.assetComparisons[1].verifiedCheckpointChangePercent}%</dd></div>
                      <div><dt>Outcomes / geometry</dt><dd>Unpublished · unplotted</dd></div>
                    </dl>
                ...
                {selectedCoverageAcceptedLandBidPrices.length > 0 && (
                  <section className="inspector-section context-section">
                    <div className="section-heading"><div><span>Outcome interpretation</span><h2>Accepted bid is not a completed sale</h2></div><CircleHelp size={17} /></div>
                    <dl className="detail-grid">
                      <div><dt>Reserve</dt><dd>₹68.00 lakh · ₹3,225.81/sq ft</dd></div>
                      <div><dt>Accepted bid</dt><dd>₹68.70 lakh · ₹3,259.01/sq ft</dd></div>
                      <div><dt>Premium</dt><dd>1.03% above reserve</dd></div>
                      <div><dt>Paid</dt><dd>25% deposit</dd></div>
                      <div><dt>Outcome</dt><dd>Cancelled · deposit forfeited</dd></div>
                      <div><dt>Registered sale</dt><dd>None verified</dd></div>
                    </dl>
                    <p className="coverage-method">The accepted bid is real demand evidence, but it never became a completed transaction because the 75% balance was not paid. The DRT dismissed the later challenge on limitation; MITO does not turn that procedural ruling into title certification.</p>
                    <a className="resolver-api-link" href={omrSemmancheriAcceptedLandBidOutcomeLedger.sources[0].url} target="_blank" rel="noreferrer">Open the official DRT docket <ArrowUpRight size={13} /></a>
                    <a className="resolver-api-link" href={omrSemmancheriAcceptedLandBidOutcomeLedger.sources[1].url} target="_blank" rel="noreferrer">Open the public judgment mirror <ArrowUpRight size={13} /></a>
                  </section>
                )}
