...
test("server-renders the MITO product shell", async () => {
  ...
  assert.match(html, /2(?:<!-- -->)? Kalavakkam villa reserve checkpoints/i);
  assert.match(html, /later unpriced schedule/i);
...
test("coverage API publishes the honest OMR release gate", async () => {
  ...
  assert.equal(payload.summary.kalavakkamVillaCombinedAssetReserveCount, 2);
  assert.equal(payload.summary.kalavakkamVillaCombinedAssetReserveAssetCount, 1);
  assert.equal(payload.summary.kalavakkamVillaCombinedAssetReserveVillageCount, 1);
  assert.equal(payload.summary.kalavakkamVillaCombinedAssetLandReservePriceCount, 0);
  assert.equal(payload.summary.kalavakkamVillaCombinedAssetWinningBidRecordCount, 0);
  assert.equal(payload.summary.kalavakkamVillaCombinedAssetRegisteredTransactionCount, 0);
  ...
  const kalavakkam = payload.evidenceMatrix.find((entry) => entry.key === "20096:1318");
  assert.equal(kalavakkam.kalavakkamVillaCombinedAssetReserves.status, "direct_combined_villa_and_uds_reserve_checkpoints_found");
  assert.equal(kalavakkam.kalavakkamVillaCombinedAssetReserves.recordCount, 2);
  assert.equal(kalavakkam.kalavakkamVillaCombinedAssetReserves.assetCount, 1);
  assert.equal(kalavakkam.kalavakkamVillaCombinedAssetReserves.directRecordCount, 2);
  assert.equal(kalavakkam.kalavakkamVillaCombinedAssetReserves.combinedLandBuildingReserveRecordCount, 2);
  assert.equal(kalavakkam.kalavakkamVillaCombinedAssetReserves.landReservePriceRecordCount, 0);
  assert.equal(kalavakkam.kalavakkamVillaCombinedAssetReserves.verifiedCheckpointReductionPercent, 14.98);
  assert.equal(kalavakkam.kalavakkamVillaCombinedAssetReserves.completeReserveHistoryVerified, false);
  assert.equal(kalavakkam.kalavakkamVillaCombinedAssetReserves.laterUnpricedLifecycleSourceCount, 1);
  assert.equal(payload.evidenceMatrix.filter((entry) => entry.kalavakkamVillaCombinedAssetReserves.recordCount === 0).length, 23);
...
test("evidence API publishes planning provenance, current register metadata and the rights gate", async () => {
  ...
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.summary.recordCount, 2);
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.summary.assetCount, 1);
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.summary.directVillageLinkCount, 1);
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.summary.landReservePriceRecordCount, 0);
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.summary.completedSaleRecordCount, 0);
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.summary.winningBidRecordCount, 0);
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.summary.registeredTransactionCount, 0);
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.summary.completeReserveHistoryVerified, false);
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.summary.laterUnpricedLifecycleSourceCount, 1);
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.location.officialVillageCode, "1318");
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.location.geometryStatus, "unplotted");
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.asset.undividedLandShareSqft, 2000);
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.asset.superBuiltUpAreaSqft, 2705);
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.records.length, 2);
  assert.ok(payload.kalavakkamVillaCombinedAssetReserves.records.every((record) => !record.isLandReservePrice && record.winningBidInr === null));
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.laterLifecycleEvidence.reservePricePublished, false);
  assert.equal(payload.kalavakkamVillaCombinedAssetReserves.publication.personalDataFieldsStored, 0);
  assert.match(payload.kalavakkamVillaCombinedAssetReserves.publicationRule, /must never be relabelled as a winning bid/i);
